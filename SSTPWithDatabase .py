...
while True:      # keep looping until we break out of the loop
    try:
        spidtestnumserver= int(input(f"how many servers of {len(ser['server'])}  to check speedTest?"))
...
# merge the speedtest with servers back into the original DataFrame
...
appenddata = pd.read_csv('long.csv')
appenddata = appenddata.append(result)
result.to_html('sstp.html', escape=False, index=False)
appenddata = appenddata.sort_values(by=['speed_test_r'], ascending=True)
